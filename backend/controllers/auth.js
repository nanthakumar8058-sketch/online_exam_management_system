...
exports.register = async (req, res, next) => {
  try {
    ...
    const generatedPassword = password ? null : crypto.randomBytes(6).toString('hex');
    ...
    if (generatedPassword) {
      try {
        await emailService.sendCredentials(email, finalPassword, name || 'Admin');
      } catch (emailErr) {
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!identifier || !password) {
      return res.status(400).json({ success: false, error: 'Please provide an email or ID, and password' });
    }
