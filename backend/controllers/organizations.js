...
exports.getOrgUsers = async (req, res) => {
    try {
        const users = await User.find({ organization: req.user.organization, role: { $in: ['staff', 'student'] } })
...
exports.registerOrgUser = async (req, res) => {
    try {
        const { name, email, role, employeeId, registerNumber, department } = req.body;
        ...
        if (role === 'staff') {
            userFields.employeeId = employeeId || `EMP-${Math.floor(100000 + Math.random() * 900000)}`;
            generatedId = userFields.employeeId;
        } else if (role === 'student') {
            userFields.registerNumber = registerNumber || `STU-${Math.floor(100000 + Math.random() * 900000)}`;
            generatedId = userFields.registerNumber;
        }
        ...
        let emailSent = false;
        try {
            await emailService.sendCredentials(email, password, name, generatedId);
            emailSent = true;
        } catch (emailErr) {
            console.error('Email failed:', emailErr.message);
        }
